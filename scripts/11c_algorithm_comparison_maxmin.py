"""
11c_algorithm_comparison_maxmin.py — Fair ALG-0/1/2 Comparison, v3 (Task 14 extension)
================================================================================================
VERSION 3 of the Task 14 comparison (see 11_algorithm_comparison.py for v1
and its full design rationale, unchanged here; 11b for v2's percentile
addition). v1's throughput-max ALG-2 (schedule_pairs_greedy) left min-pair
key at exactly 0 in 83-88% of draws. This version runs a SECOND ALG-2
variant, schedule_pairs_greedy_maxmin (fairness-oriented, water-filling
greedy that always helps the currently-worst-off pair first), ALONGSIDE
the original throughput-max variant on every draw, to quantify the
throughput-vs-fairness trade-off directly: how much total sum(K_q) is
given up to raise the floor min(K_q)?

Outputs go to temp/v3_maxmin/ (kept separate from v1 and v2 per user
request to preserve all three analysis versions distinctly).
...
Run:
  cd 05_Code_v2 && python scripts/11c_algorithm_comparison_maxmin.py
"""
import os
import sys
import time
...
TEMP_DIR = os.path.join(os.path.dirname(__file__), '..', 'temp', 'v3_maxmin')
...
with open(RAW_RUNS_CSV, 'w', newline='', encoding='utf-8') as f:
    csv.writer(f).writerow([
        ...
        # v3 additions: fairness-oriented ALG-2 variant run alongside the
...
for season, month in SEASONS.items():
    ...
    for run in range(N_RUNS_PER_SEASON):
        ...
        min_pair_key = min(totals2.values()) if totals2 else 0.0
        ...
        # temp/v3_maxmin/raw_runs.csv immediately, so partial results are
...
save_intermediate_csv(table_rows, "algorithm_comparison_summary_v3_maxmin",
                       "v3: Mean +/- std network key (Gbit/day) per algorithm per season, "
                       "including ALG-2-maxmin (fairness-oriented), 200 MC draws")

# v3: throughput-vs-fairness trade-off table
tradeoff_rows = []
for season in SEASONS:
...
save_intermediate_csv(tradeoff_rows, "algorithm_comparison_tradeoff_v3_maxmin",
                       "v3: throughput-max vs fairness-max ALG-2 trade-off "
                       "(sum given up to raise the pair-key floor)")
...
plt.suptitle('Algorithm Comparison incl. Fairness Variant (200 real-day MC draws)',
             fontsize=9, fontweight='bold')
plt.tight_layout()
out1 = os.path.join(FIG_DIR, 'fig08c_algorithm_comparison_cdf_v3.png')
...
save_verify_numbers(verify, "algorithm_comparison_v3_maxmin")

save_provenance(
    script_name="11c_algorithm_comparison_maxmin",
