"""
11_algorithm_comparison.py — Fair ALG-0/1/2 Comparison (Task 14, plan 07-5)
================================================================================
...
Run:
  cd 05_Code_v2 && python scripts/11_algorithm_comparison.py
"""
import os
import sys
import time
...
for season, month in SEASONS.items():
    ...
    for run in range(N_RUNS_PER_SEASON):
        ...
        # temp/raw_runs.csv immediately, so partial results are inspectable
        # at any time even if the run is interrupted or still in progress.
        append_run_row([
...
save_intermediate_csv(table_rows, "algorithm_comparison_summary",
                       "Mean +/- std network key (Gbit/day) per algorithm per season, "
...
plt.tight_layout()
out1 = os.path.join(FIG_DIR, 'fig08_algorithm_comparison_cdf.png')
...
save_verify_numbers(verify, "algorithm_comparison")

save_provenance(
    script_name="11_algorithm_comparison",
