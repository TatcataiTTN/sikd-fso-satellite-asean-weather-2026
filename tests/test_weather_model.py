...
class TestDataRanges:
    ...
    def test_rainfall_reasonable_max(self):
        """R_mm_month ≤ 700 mm — giới hạn vật lý hợp lý cho nhiệt đới
        (Đà Nẵng/Huế tháng 10 mùa bão lũ ghi nhận thật >500mm/tháng;
        bound 700mm chừa biên độ trong khi vẫn bắt được lỗi dữ liệu)."""
        for city in ALL_CITIES:
            for i, (R, V, P) in enumerate(ASEAN_CLIMATE_DATA[city]):
                assert R <= 700, (
                    f"{city} month {i+1}: R={R} mm/month > 700 (unrealistic)"
                )
